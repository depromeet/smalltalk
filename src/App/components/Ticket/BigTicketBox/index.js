...
import styles from './BigTicketBox.module.scss';

const cx = classnames.bind(styles);

class BigTicketBox extends Component{
  constructor(){
    super();
    this.state = {
      clickedTicket: []
  ...
  makeBigTicketList = ( ticketList ) => { 
    return ticketList.map(( ticket, i ) => {
    // console.log(ticket);
      return (
        <li  key={`Bigticket-${i}`} 
        className={cx("t_item", (this.state.clickedTicket.findIndex(x => x === ticket.id) === -1? 'none' : 'active'))} 
  ...
  render(){
    ...
    return(
      <ul className={cx('call_t')} style={this.resizeWidth(ticketList.length)}>
        { this.makeBigTicketList(ticketList) }
      </ul>
    );
  }
}

export default BigTicketBox;