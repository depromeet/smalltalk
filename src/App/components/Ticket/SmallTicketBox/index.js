...
import styles from './SmallTicketBox.module.scss';

const cx = classnames.bind(styles);

class SmallTicketBox extends Component{
  constructor(){
    super();
    this.state = {
      isToggleOn: 0,
  ...
  makeSmallTicketList = ( ticketList ) => { 
    return ticketList.map(( ticket, i ) => {
      if(this.state.isToggleOn === i) { 
        return ( 
          <li key={`SmallTicket-${i}`} onClick={() => this.activeTicket(i)}>
            <SmallTicket 
              listLen = { ticketList.length } 
              ticket = { ticket } 
              backColor = { this.state.cirColor } 
              cirColor = { this.state.backColor }
            />
          </li> 
        ); 
      }
      ...
      <div className={cx('arrow_list')}>
        <div className={cx('t_box')}>
          <ul className={cx('call_t')} style={this.resizeWidth(ticketList.length)}>
            {this.makeSmallTicketList(ticketList)}
          </ul>
        </div>
        <div className={cx('paddle-nav')}>
          <ul>
            <li className={cx('arrow', 'prev')} 
              onClick={() => this.prevEnt(ticketList.length)} 
              style={this.prevEntHidden()}>
              <button>&lt;</button>
            </li>
            <li className={cx('arrow', 'next')} 
              onClick={() => this.nextEnt(ticketList.length)} 
              style={this.nextEntHidden(ticketList.length)}>
              <button>&gt;</button>
            </li>
          </ul>
        </div>
